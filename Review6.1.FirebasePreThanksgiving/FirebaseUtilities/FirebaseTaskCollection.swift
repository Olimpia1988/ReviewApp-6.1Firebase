...
class FirestoreService {
    ...
    func getAllPosts(completion: @escaping (Result<[Task], Error>) -> ()) {
        db.collection(FireStoreCollections.tasks.rawValue).getDocuments { (snapshot, error) in
            if let error = error {
                completion(.failure(error))
            } else {
                let posts = snapshot?.documents.compactMap({ (snapshot) -> Task? in
                    let postID = snapshot.documentID
                    let post = Task(from: snapshot.data(), id: postID)
                    return post
                })
    ...
    func getPosts(forUserID: String, completion: @escaping (Result<[Task], Error>) -> ()) {
        db.collection(FireStoreCollections.tasks.rawValue).whereField("creatorID", isEqualTo: forUserID).getDocuments { (snapshot, error) in
            if let error = error {
                completion(.failure(error))
            } else {
                let posts = snapshot?.documents.compactMap({ (snapshot) -> Task? in
